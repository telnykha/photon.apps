...
//	m_pam2Doc.EndRecording();
	...
	if (Application->MessageBoxW(L"Вы уверены, что хотите прервать запись?", L"PAM2", MB_YESNO)== IDYES)
	{
		pam2ExperimentForm->Gauge1->Progress = 0;
		m_currentFlash = 0;
		this->ExecuteCommand(L"OFF");

		if (Application->MessageBoxW(L"Сохранить измерения?", L"PAM2", MB_YESNO)== IDYES)
		{
...
void __fastcall TpamMainForm::Timer1Timer(TObject *Sender)
{
	...
	pam2ExperimentForm->StatusBar1->Panels->Items[1]->Text = L"Кадр: " + IntToStr(m_currentFlash);
...
void __fastcall TpamMainForm::ChangeRoi(TObject* sender, int index, bool update)
{
...
//		printf("Имя порта: %s\n", phserialPortName(buf, i));
		...
		AnsiString _str =phserialPortSerialNumber(buf, i);
			if (_str == "55732323530351212221") {
				 result =  _strLocation;
				 break;
			}
	}
