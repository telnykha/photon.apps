...
extern "C" AWPCVVIDEO_API HRESULT awpcvChessboard(awpImage* image, awp2DPoint* points)
{
	...
	if (img == NULL)
		return E_FAIL;
	CvSize patternsize(8, 6);
	CvPoint2D32f cv_points[48];
	bool patternfound = cvFindChessboardCorners(img, patternsize, cv_points);
	if (patternfound)
	{
		for (int i = 0; i < 48; i++)
		{
			points[i].X = cv_points[i].x;
			points[i].Y = cv_points[i].y;
		}
	}
	cvReleaseImage(&img);
	return patternfound ? S_OK: E_FAIL;
}
