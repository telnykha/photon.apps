...
void __fastcall TCommandsTable::LoadTable(const char* lpFileName)
{
    ...
    for(TiXmlNode* child = e->FirstChild(); child; child = child->NextSibling() )
    {
         ...
         const int row_count = m_grid->RowCount;
          ...
          m_grid->Cells[1][m_grid->RowCount-1] = GetCommandIntensivity(event->pinStatus, event->command);
     ...
     try
      {
        ...
        m_grid->Cells[1][m_grid->RowCount-1] = GetCommandIntensivity(dlg->ComboBox2->ItemIndex, dlg->ComboBox1->ItemIndex);
...
void __fastcall TCommandsTable::EditRecord(int index, TeditorDlg* dlg)
{
        ...
        m_grid->Cells[1][index] = GetCommandIntensivity(dlg->ComboBox2->ItemIndex, dlg->ComboBox1->ItemIndex);
...
UnicodeString TCommandsTable::GetCommandName(int index)
{
    ...
    {
        case 0:
            return L"Включить прожектор 460 nm";
        case 1:
            return L"Включить прожектор 660 nm";
        case 2:
            return L"Включить прожектор 660 nm и фото";
        case 3:
            return L"Получить изображение";
        case 4:
            return L"Выключить все";
        case 5:
            return L"Включить прожектор 460 nm и фото";
        default:
...
UnicodeString __fastcall TCommandsTable::GetScript()
{
    ...
    for (int i = 0; i < m_list->Count; i++)
    {
        ...
        str += L",";
        str += IntToStr(GetCommandIntensivityValue(e->pinStatus));//L"HIGH,";
        str += L",";
